...
AmbientVacuum::AmbientVacuum()
{
	...
	//Al azar
	m_aRoomDity[0] = rand()%2;
	m_aRoomDity[1] = rand()%2;
	m_aRoomDity[2] = rand()%2;
	m_aRoomDity[3] = rand()%2;

	//Pero una sucia siempre
	m_aRoomDity[rand()%2] = true;
	

	
	//m_OriginalRoomIndex = m_RoomIndex = 1;
	m_OriginalRoomIndex = m_RoomIndex = rand()%4+1;
}
